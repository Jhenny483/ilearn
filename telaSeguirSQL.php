...
// var_dump($idSeguido);
		...
		if( $res == TRUE){
			...
			echo "<script language='javascript' type='text/javascript'>alert('Você já segue $nomeUsu');window.location.href='telaIndex.php';</script>";
		
		} else {
		
			$insert = "INSERT INTO seguidor (idSeguido, idSeguidor) VALUES ('{$idSeguido}' , '{$_SESSION['cod'][0]}')";
				$banco->executarSQL($insert);
			echo "<script language='javascript' type='text/javascript'>alert('Você seguiu $nomeUsu');window.location.href='telaIndex.php';</script>";
